...
The **Episodes** tab adds the canonical, private Fast H3 compose-and-play workflow to the root application. Signed-in users can draft an episode from an idea with the optional server-side AI writer or write scenes by hand, then queue one to six scenes as a continuous video. The tab supports manual Connect/Disconnect controls while retaining lazy connect on Queue episode, capacity-checks the full episode, chains each later scene from the previous clip's final frame, mirrors both model queues, and supports synchronized playback and a 10-second MP4 capture.
