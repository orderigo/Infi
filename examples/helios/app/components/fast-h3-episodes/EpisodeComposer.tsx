...
import {
  ...
  EXAMPLE_EPISODE,
  MAX_SCENES,
} from "../../lib/fast-h3-episodes/prompts";
import { makeTag } from "../../lib/fast-h3-episodes/tag";
...
export function EpisodeComposer() {
  ...
  useEffect(() => {
    // Whether /api/fast-h3/upsample has a key behind it decides which compose path
    // the UI leads with.
    fetch("/api/fast-h3/upsample", { cache: "no-store" })
      .then((r) => r.json())
  ...
  async function writeWithAi() {
    ...
    setError(null);
    try {
      const response = await fetch("/api/fast-h3/upsample", {
        method: "POST",
